    ...
    *arXiv preprint* arXiv:1705.08527.
    ...
    def __init__(self, network_list, exposure, outcome, degree_restrict=None, alpha=0.05, _gs_measure_='sum',
        ...
        if degree_restrict is not None:                                                         # not-None means apply a restriction
            ...
            self._max_degree_list_ = [np.int64(degree_restrict[1])]*len(network_list)           # ... extract max degree as upper bound
    ...
    def outcome_model_UDA(self, model, custom_model=None, 
        ...
        if self.parallel_id is None:
            ...
            custom_path = 'outcome_' + self.outcome + '_' + self.task_string + '_' + str(self.parallel_id) +'.pth'
        self._q_custom_ = custom_model
    ...
    def fit(self, p, samples=100, bound=None, seed=None,
        ...
        if self.use_deep_learner_outcome: # set DL epsilon with bounded q_init in targeting_step()
            # epsilon = 0.
            # for restricted data, the number of samples in the last time step can be different from the predict self._Qinit_DL_ 
            # because self._Qinit_DL_ is clipped based on the minimum of src_data and trg_data sample numbers
            num_sample_last_step = self.df_restricted_list[-1].shape[0]
            num_sample_predicted = self._Qinit_DL_.shape[0]
            num_sample_iptw = h_iptw.shape[0]
            min_sample = min(num_sample_last_step, num_sample_predicted)
            print(f'df_restricted_list[-1] sample number: {num_sample_last_step},  _Qinit_DL_ sample number: {num_sample_predicted}, h_iptw sample number: {num_sample_iptw}, min_sample: {min_sample}')
        
            epsilon = targeting_step(y=self.df_restricted_list[-1][self.outcome][:min_sample],   # Estimate the targeting model given observed Y
                                     q_init=self._Qinit_DL_[:min_sample],                        # ... predicted values of Y under observed A
                                     ipw=h_iptw[:min_sample],                                    # ... weighted by IPW
        ...
        if self.use_deep_learner_outcome:
            var_cond = self._est_variance_conditional_(iptw=h_iptw[:min_sample],                                 # Estimate direct-only variance
            ...
            var_lcond = self._est_variance_latent_conditional_(iptw=h_iptw[:min_sample],                         # Estimate latent variance
                                                            ...
                                                            excluded_ids=excluded_ids_for_sample_matching)
